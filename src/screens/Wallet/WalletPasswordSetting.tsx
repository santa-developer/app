 ...
 * 인증비밀번호 최초설정 화면
 */
export default function WalletPasswordSetting({
  route,
}: any): JSX.Element {
  const [input, setInput] = useState<number[]>([])
  const { from, sendHIBS } = route.params

  useEffect(() => {
    if (input.length === 6) {
      NavigationService.navigate('WalletPasswordSettingSecond', {
        password: input.join(),
        from,
        sendHIBS,
      })
...
WalletPasswordSetting.navigationOptions =
