...
export default function LikePosts(): React.JSX.Element {
  ...
  function LikePost({
    ...
    return (
      <PostBox>
        ...
        <PostBoxBottom>
          <IconHeart width={12} height={12} svgColor={Colors.error} />
          <Text
            size={13}
  ...
  return (
    <Body style={{ backgroundColor: Colors.gr }} hidePadding>
      <LikeInfo>
        <Text size={16}>좋아요 받은 게시물</Text>
        <LikePntInfo onPress={showModal}>
          {nowData?.weekPoitRegYn === 'Y' ? (
            <Text
              ...
              color={Colors.point}
            >
              +{Number(atvtPoint?.newLikeRPnt)}P
            </Text>
          ) : (
            <Text
              style={{ marginRight: 5 }}
...
const LikePntInfo = styled.TouchableOpacity`
  flex-direction: row;
  align-items: center;
`
